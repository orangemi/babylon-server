...
Task.prototype.assignToTask = function(task, sort, options, next) {
	...
	Then(function(then) {
		...
		//UPDATE sort
		var sql1 = "UPDATE task2task SET sort=sort+1 WHERE parent_id = ? AND sort >= ? AND task_id <> ?";
		var sql2 = "UPDATE task2task SET sort=sort-1 WHERE parent_id = ? AND sort >= ? AND task_id <> ?";
		var sql = !options.del ? sql1 : sql2;
...
Task.prototype.assignToPerson = function(person, sort, options, next) {
	...
	Then(function(then) {
		...
		//UPDATE sort
		var sql1 = "UPDATE person2task SET sort=sort+1 WHERE person_id = ? AND sort >= ? AND task_id <> ?";
		var sql2 = "UPDATE person2task SET sort=sort-1 WHERE person_id = ? AND sort >= ? AND task_id <> ?";
		var sql = !options.del ? sql1 : sql2;
