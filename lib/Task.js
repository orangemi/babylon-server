...
Task.findByUser = function(user, organization, options, next) {
	...
	var sql = "SELECT * FROM task WHERE organization_id = ? AND assign = ? ORDER BY sort ASC, complete ASC";
	Data.query(sql, [organization.id, user.id], function(err, rows) {
...
Task.findByTask = function(task, options, next) {
	...
	var sql1 = "SELECT task.* FROM task2task LEFT JOIN task ON task.id = task2task.task_id WHERE task2task.parent_id = ? AND task.organization_id = ? AND task.id IS NOT NULL ORDER BY sort ASC";
	var sql2 = "SELECT task.* FROM task2task LEFT JOIN task ON task.id = task2task.parent_id WHERE task2task.task_id = ? AND task.organization_id = ? AND task.id IS NOT NULL ORDER BY sort ASC";
	var sql;
	switch (options.type) {
	...
	Data.query(sql, [task.id, task.organization_id], function(err, rows) {
		if (err) return next(err);
		var tasks = [];
		rows.forEach(function(row) {
			var task = new Task();
...
Task.prototype.assignToUser = function(user, sort, options, next) {
	if (typeof(options) == 'function') {
		next = options;
		options = {};
	}
	...
	if (!this.id) throw new Error('no task id');
	if (user && !(user instanceof User)) throw new Error('user is not a User');
	var self = this;
	var assign = user ? user.id : 0;
	sort = sort || 1;
